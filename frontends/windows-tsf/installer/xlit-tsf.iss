...
;
; Uninstall fully reverses this: language-list entry (+ ne-NP if now empty),
; regsvr32 /u for each bitness, an HKLM key force-delete fallback, an HKCU
; CTF cache sweep, and Inno's own file/registry/ARP removal.
...
const
  NeTag = 'ne-NP';
  // must match register.rs / lib.rs
  NeClsid = '{438E43E4-3800-4AB1-82A6-A2E831ABF107}';
  // langid : {CLSID}{profile GUID}
...
{ ...and removes it again: the language-list entry (drop ne-NP if nothing else
  is left under it) plus any cached HKCU CTF entries that still name the CLSID. }
...
begin
  Result :=
    ...
    '$n=$l | Where-Object { $_.LanguageTag -eq ''' + NeTag + ''' }' + #13#10 +
    'if ($n) {' + #13#10 +
    '  foreach ($m in @($n.InputMethodTips | Where-Object { $_ -ieq $t })) { [void]$n.InputMethodTips.Remove($m) }' + #13#10 +
    '  if ($n.InputMethodTips.Count -eq 0) { [void]$l.Remove($n) }' + #13#10 +
    '  Set-WinUserLanguageList $l -Force' + #13#10 +
    '}' + #13#10 +
    'foreach ($root in ''HKCU:\SOFTWARE\Microsoft\CTF\Assemblies'',''HKCU:\SOFTWARE\Microsoft\CTF\SortOrder\AssemblyItem'') {' + #13#10 +
    '  if (Test-Path $root) {' + #13#10 +
    '    $hits = foreach ($k in (Get-ChildItem $root -Recurse -ErrorAction SilentlyContinue)) {' + #13#10 +
    '      try {' + #13#10 +
    '        if ($k.PSChildName -like "*$c*") { $k.PSPath; continue }' + #13#10 +
    '        foreach ($v in $k.Property) { if ("$($k.GetValue($v))" -like "*$c*") { $k.PSPath; break } }' + #13#10 +
    '      } catch {}' + #13#10 +
    '    }' + #13#10 +
    '    $hits | Select-Object -Unique | ForEach-Object { Remove-Item $_ -Recurse -Force -ErrorAction SilentlyContinue }' + #13#10 +
    '  }' + #13#10 +
    '}';
end;
...
begin
  ...
  begin
    { HKLM force-delete fallback in case a DLL could not be loaded to
      self-unregister (missing file, wrong bitness of regsvr32, ...). }
    RegDeleteKeyIncludingSubkeys(HKLM, 'SOFTWARE\Classes\CLSID\' + NeClsid);
    RegDeleteKeyIncludingSubkeys(HKLM, 'SOFTWARE\Classes\Wow6432Node\CLSID\' + NeClsid);
    RegDeleteKeyIncludingSubkeys(HKLM, 'SOFTWARE\Microsoft\CTF\TIP\' + NeClsid);
    RecycleInputHosts;
  end;
end;
